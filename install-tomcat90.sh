...
# download URL
URL=http://apache.spinellicreations.com/tomcat/tomcat-9/v$VERSION/bin/apache-tomcat-$VERSION.tar.gz
#    http://mirror.jax.hugeserver.com/apache/tomcat/tomcat-8/v$VERSION/bin/apache-tomcat-$VERSION.tar.gz
#							    v8.5.30/bin/apache-tomcat-8.5.30.tar.gz
...
if [ ! -f apache-tomcat-$VERSION.tar.gz ]; then
	...
	echo Check https://tomcat.apache.org/download-90.cgi
...
cd apache-tomcat-$VERSION || exit 3
...
if [ -f   conf/tomcat-users.xml.bak ]; then
  ...
  perl -pi.bak -e 'if ( m=</tomcat-users>= ) { print "<role rolename=\"manager-gui\"/>\n<user username=\"tomcat\" password=\"tomc90pw\" roles=\"manager-gui\"/>\n"; }' \
  conf/tomcat-users.xml
  echo
  sleep 1
  echo ====================== NOTE
  echo =========================== manager-gui role added for tomcat/tomc90pw
  sleep 1
  echo
  echo heres the diff:
  sleep 1
...
sleep 1
# in an error loop this will possibly go on forever...
