...
# download URL
URL=http://mirror.jax.hugeserver.com/apache/tomcat/tomcat-8/v$VERSION/bin/apache-tomcat-$VERSION.tar.gz
...
if [ ! -f apache-tomcat-$VERSION.tar.gz ]; then
	...
	echo Check https://tomcat.apache.org/download-80.cgi
...
if [ -f   conf/tomcat-users.xml.bak ]; then
  ...
  perl -pi.bak -e 'if ( m=</tomcat-users>= ) { print "<role rolename=\"manager-gui\"/>\n<user username=\"tomcat\" password=\"tomc85pw\" roles=\"manager-gui\"/>\n"; }' \
  conf/tomcat-users.xml
  echo
  sleep 1
  echo ====================== NOTE
  echo =========================== manager-gui role added for tomcat/tomc85pw
  sleep 1
  echo
  echo heres the diff:
  sleep 1
